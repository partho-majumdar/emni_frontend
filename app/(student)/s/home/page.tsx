...
import { MoreVertical, Vote, Send, Check, Trash2, Edit3, X, Search } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const NewsFeed: React.FC = () => {
  ...
  const getCurrentUserId = () => {
    ...
    if (studentId) {
      return { id: studentId, type: "student" as const };
    }
    return null;
  };
  ...
  const fetchCurrentUser = useCallback(async () => {
    ...
    try {
      ...
      if (!user?.id || user.type !== "student") {
        throw new Error("Please sign in as a student to access the news feed");
      }

      const profileDetails = (await getMyProfileDetailsStudent()) as StudentInfoType;
      if (!profileDetails) {
      ...
      setCurrentUser({
        ...
        image_link: profileDetails.image_link || getAvatar(profileDetails.username),
      });
    } catch (err) {
    ...
    try {
      ...
      if (pollRes.success) {
        ...
        const normalizedPolls = pollsData.map((poll: any) => ({
          ...
          user_type: poll.user_type || "Student",
  ...
  const handleDeletePost = async (post_id: string) => {
    setDeleteModal({ isOpen: true, itemId: post_id, itemType: "Post" });
  };

  const handleDeletePoll = async (poll_id: string) => {
    setDeleteModal({ isOpen: true, itemId: poll_id, itemType: "Poll" });
  };
  ...
  const confirmDelete = async () => {
    ...
    setIsSubmitting(true);
    try {
      const isPost = deleteModal.itemType === "Post";
      const endpoint = isPost
        ? `api/news-feed/post/${deleteModal.itemId}`
        : `api/news-feed/poll/${deleteModal.itemId}`;
      const updateState = isPost
        ? () => setPosts(prev => prev.filter(post => post.post_id !== deleteModal.itemId!))
        : () => setPolls(prev => prev.filter(poll => poll.poll_id !== deleteModal.itemId!));
      const revertState = isPost
        ? (prev: Post[]) => setPosts(prev)
        : (prev: Poll[]) => setPolls(prev);
      ...
      const res = await apiRequest(req);
      if (res.success) {
        toast.success(`${deleteModal.itemType} deleted successfully`);
        setDeleteModal({ isOpen: false, itemId: null, itemType: null });
      } else {
        if (isPost) {
          (revertState as (prev: Post[]) => void)(posts);
        } else {
          (revertState as (prev: Poll[]) => void)(polls);
        }
        throw new Error(res.message || `Failed to delete ${deleteModal.itemType.toLowerCase()}`);
      }
    } catch (err) {
      const error = err as Error;
      console.error(`Failed to delete ${deleteModal.itemType?.toLowerCase()}:`, error.message);
      toast.error(error.message || `Error deleting ${deleteModal.itemType?.toLowerCase()}`);
    } finally {
      setIsSubmitting(false);
    }
  };
                            ...
                            {(item.user_id === currentUser?.user_id || selectedFilter === "my-posts") && (
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent className="w-40 bg-slate-800/95 border border-slate-700/50 text-white">
                                  ...
                                  <DropdownMenuItem
                                    className="flex items-center space-x-2 text-red-400 hover:bg-slate-700/50"
                                    onClick={() => handleDeletePoll(item.poll_id)}
                                  >
                                    <Trash2 className="w-4 h-4" />
                                    <span>Delete</span>
                                  </DropdownMenuItem>
...
// import { Button } from "@/components/ui/button";
// import { Badge } from "@/components/ui/badge";
// import { MoreVertical, Vote, Send, Check, Trash2, Edit3, X } from "lucide-react";
// import {
//   DropdownMenu,
//   DropdownMenuContent,
//   DropdownMenuItem,
