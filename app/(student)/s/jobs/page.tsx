...
import { Briefcase, Send, Trash2, Edit3, Calendar, Coins, Users, Clock, Search } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const JobsPage: React.FC = () => {
  ...
  const [selectedFilter, setSelectedFilter] = useState<"my-jobs" | "other-jobs">("other-jobs");
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      const endpoint =
        selectedFilter === "my-jobs"
          ? "api/student/jobs/my-jobs"
          : "api/student/s/jobs?status=Open";
  ...
  const fetchMyApplications = useCallback(async () => {
    ...
    try {
      const res = await apiRequest({
        endpoint: "api/student/jobs/my-applications",
        method: "GET",
        auth: true,
      });
      if (res.success) {
    ...
    if (term.trim() === "") {
      ...
      const filtered = jobs.filter(
        (job) =>
          ...
          job.poster_username.toLowerCase().includes(lowerTerm)
      );
  ...
  const handleUpdateJob = async () => {
    ...
    try {
      const res = await apiRequest({
        endpoint: `api/student/jobs/${job_id}`,
        method: "PUT",
        body: {
          title,
          description,
  ...
  const handleApplyJob = async () => {
    ...
    try {
      const res = await apiRequest({
        endpoint: `api/student/jobs/${applyForm.jobId}/apply`,
        method: "POST",
        body: {
          description: applyForm.description,
          email: applyForm.email,
          phone_number: applyForm.phone_number,
        },
        auth: true,
      });
      if (res.success) {
        toast.success("Application submitted successfully");
        setApplyForm({ jobId: "", description: "", email: "", phone_number: "" });
        await Promise.all([fetchJobs(), fetchMyApplications()]);
      } else {
        throw new Error(res.message || "Failed to apply");
      }
    } catch (err: any) {
      toast.error(err.message || "Error applying for job");
      console.error(err);
    } finally {
      setIsSubmitting(false);
    }
  ...
  useEffect(() => {
    if (studentId) {
      Promise.all([fetchJobs(), fetchMyApplications()]);
    }
  }, [fetchJobs, fetchMyApplications, studentId]);
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm mt-1">
                    Discover opportunities, create connections
                  </p>
                </div>
              </div>
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-4 bg-slate-800/30 backdrop-blur-sm rounded-xl p-2 border border-slate-700/30">
                  <span className="text-sm font-medium text-slate-300">Other Jobs</span>
                  <Switch
                    checked={selectedFilter === "my-jobs"}
                    onCheckedChange={(checked) =>
                      setSelectedFilter(checked ? "my-jobs" : "other-jobs")
                    }
                    className="data-[state=checked]:bg-orange-500"
                  />
                  <span className="text-sm font-medium text-slate-300">My Jobs</span>
                </div>
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="relative bg-slate-800/30 backdrop-blur-sm border border-slate-700/30 rounded-2xl p-10 text-center">
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">
                      {selectedFilter === "my-jobs" ? "No jobs created yet" : "No jobs available"}
                    </h3>
                    <p className="text-slate-400 text-sm">
                      {selectedFilter === "my-jobs" ? "Create your first job opportunity above" : "Check back later or adjust your search"}
                    </p>
                  </div>
                </motion.div>
              </div>
              ...
              filteredJobs.map((job, index) => (
                ...
                >
                  ...
                  <div className="relative bg-slate-800/40 backdrop-blur-sm border border-slate-700/30 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:border-slate-600/50">
                    ...
                    {/* Poster Info */}
                    {selectedFilter === "other-jobs" && (
                      <div className="border-t border-slate-700/50 pt-4 mb-4">
                        <div className="flex items-center space-x-3">
                          <div className="relative">
                            <Image
                              src={getAvatar(job.poster_username)}
                              alt={`${job.poster_username}'s avatar`}
                              width={40}
                              height={40}
                              className="rounded-full border-2 border-orange-500/50 shadow-md"
                            />
                            <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 rounded-full border border-slate-800" />
                          </div>
                          <div>
                            <p className="text-sm font-medium text-white">{job.poster_name}</p>
                            <p className="text-xs text-slate-400">@{job.poster_username}</p>
                            <p className="text-xs text-slate-400">{job.poster_user_type}</p>
                            <p className="text-xs text-slate-500">{job.poster_email}</p>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div>
                            ...
                            <p className="font-medium text-xs">{job.max_participants}</p>
                          </div>
                        </div>
                        {selectedFilter === "my-jobs" && (
                          <div className="flex items-center space-x-2">
                    ...
                    <div className="flex space-x-2">
                      {selectedFilter === "my-jobs" ? (
                        <>
                          <Button
                            onClick={() => {
                              console.log("Navigating to job:", job.job_id); // Debug log
                              router.push(`/s/jobs/${job.job_id}`);
                            }}
                        ...
                        !myApplications.some((app) => app.job_id === job.job_id) ? (
                          <Dialog>
                            <DialogTrigger asChild>
                              <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium text-sm rounded-lg h-9 transition-all duration-200 shadow-md hover:shadow-blue-500/25">
                            ...
                            <DialogContent className="bg-slate-800/40 backdrop-blur-sm border border-slate-700/30 text-white rounded-2xl">
                              ...
                              <DialogFooter className="mt-4">
                                <Button
                                  onClick={() => setApplyForm({ jobId: "", description: "", email: "", phone_number: "" })}
                                  className="bg-slate-700 hover:bg-slate-600 text-white font-medium text-sm rounded-lg h-9 transition-all duration-200 shadow-md hover:shadow-slate-500/25"
                                >
                                  Cancel
                                </Button>
                          ...
                          selectedFilter === "other-jobs" &&
                          myApplications.some((app) => app.job_id === job.job_id) && (
                            <Button disabled className="w-full bg-slate-700/50 text-slate-400 font-medium text-sm rounded-lg h-9 transition-all duration-200">
                              Already Applied
                            </Button>
                          )
                        )
