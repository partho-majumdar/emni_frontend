  ...
  useEffect(() => {
    if (selectedConversation && !selectedConversation.messages) {
      const fetchMessages = async () => {
        try {
          const request: ApiRequestType = {
            endpoint: `api/messages/conversations/mentor/${selectedConversation.mentor.mentor_id}`,
            method: "GET",
            auth: true,
          };
          const response = await apiRequest(request) as ApiResponse;
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6 relative z-10">
            ...
            <div className="space-y-3">
              {conversations.length === 0 ? (
                ...
                conversations.map((conv, index) => (
                  ...
                  >
                    <img
                      src={conv.mentor.profileImage || getAvatar(conv.mentor.username)}
                      alt={`${conv.mentor.name}'s avatar`}
                      className="w-12 h-12 rounded-full object-cover ring-2 ring-orange-500/20"
                      onError={(e) => { 
                        e.currentTarget.src = getAvatar(conv.mentor.username + "-fallback");
                        e.currentTarget.onerror = null;
                      }}
                      onClick={(e) => {
                        e.stopPropagation();
                        window.location.href = `http://localhost:5000/s/mprofile/${conv.mentor.mentor_id}`;
                      }}
                    />
                    <div className="flex-1">
                      <div className="flex justify-between items-center">
                        <p className="font-semibold text-white">{conv.mentor.name} (@{conv.mentor.username})</p>
                        {conv.unread_count > 0 && (
        ...
        >
          ...
          <div className="flex-1 overflow-hidden">
            <ScrollArea className="h-full w-full [&>div]:h-full [&>div>div]:!block">
              <div className="space-y-3 pr-3 h-full">
                {conversations.length === 0 ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center">
                          <p className="font-semibold text-white truncate">
                            {conv.mentor.name} (@{conv.mentor.username})
                          </p>
                          {conv.unread_count > 0 && (
                            <span className="bg-orange-500 text-white text-xs font-semibold rounded-full px-2 py-1 flex-shrink-0">
                              {conv.unread_count}
                            </span>
                          )}
                        </div>
            ...
            <>
              <div className="flex justify-between items-center mb-6 border-b border-orange-500/20 pb-4">
                <div className="flex items-center gap-3">
                  <img
                    src={selectedConversation.mentor.profileImage || getAvatar(selectedConversation.mentor.username)}
                    alt={`${selectedConversation.mentor.name}'s avatar`}
                    className="w-10 h-10 rounded-full object-cover ring-2 ring-orange-500/20"
                    onError={(e) => { 
                      e.currentTarget.src = getAvatar(selectedConversation.mentor.username + "-fallback");
                      e.currentTarget.onerror = null;
                    }}
                    onClick={() => window.location.href = `http://localhost:5000/s/mprofile/${selectedConversation.mentor.mentor_id}`}
                  />
                  <h3 className="text-xl font-bold text-white">
                    Chat with {selectedConversation.mentor.name} (@{selectedConversation.mentor.username})
                  </h3>
                </div>
                <motion.button
                  whileTap={{ scale: 0.9 }}
                  ...
                  {selectedConversation.messages?.length ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center h-full">
                      ...
                      <p className="text-gray-300 text-lg text-center">
                        Start your conversation with {selectedConversation.mentor.name}
                      </p>
                    </div>
                  )}
                </div>
