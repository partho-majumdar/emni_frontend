...
const MentorActiveContractsPage: React.FC = () => {
  const router = useRouter();
  const [activeContracts, setActiveContracts] = useState<{
    posted_jobs: PostedJob[];
  }>({ posted_jobs: [] });
  const [isLoading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState<string | null>(null); // Track job_id being submitted
  ...
  const fetchActiveContracts = useCallback(async () => {
    ...
    try {
      const res = await apiRequest({
        endpoint: "api/mentor/m/jobs/active-contracts",
        method: "GET",
        auth: true,
      });
      if (res.success) {
  ...
  const handleCompleteJob = async (jobId: string) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        await fetchActiveContracts(); // Refresh the contracts list
      } else {
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm mt-1">
                  View your posted jobs with hired applicants
                </p>
              </div>
            </div>
          </div>
            ...
            {isLoading ? (
              ...
              >
                ...
                <div className="relative bg-slate-800/30 backdrop-blur-sm border border-slate-700/30 rounded-2xl p-10 text-center">
                  ...
                  <p className="text-slate-400 text-sm">Create a job in the Jobs Hub or hire applicants to get started</p>
                </div>
              </motion.div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {activeContracts.posted_jobs.map((job, index) => {
                  const canCompleteJob = job.job_details.end_date && new Date() > new Date(job.job_details.end_date);
                  const showCompleteButton = job.job_details.status === "Closed" && job.job_details.hired_count > 0;
                    ...
                    >
                      ...
                      <div className="relative bg-slate-800/40 backdrop-blur-sm border border-slate-700/30 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 group-hover:border-slate-600/50">
                        ...
                        <div className="flex space-x-3">
                          <Button
                            onClick={() => router.push(`/m/jobs/${job.job_details.job_id}`)}
                          ...
                          {showCompleteButton && (
                            <div className="flex flex-col items-end space-y-2">
                              <Button
                                onClick={() => handleCompleteJob(job.job_details.job_id)}
                                disabled={isSubmitting === job.job_details.job_id || !canCompleteJob}
                                className={`flex-1 h-9 rounded-lg transition-all duration-200 shadow-md ${
                                  isSubmitting === job.job_details.job_id || !canCompleteJob
                              ...
                              >
                                {isSubmitting === job.job_details.job_id ? (
                                  <div className="flex items-center space-x-2">
                              ...
                              {!canCompleteJob && (
                                <p className="text-xs text-slate-400">
                                  Job can be completed after {formatDate(job.job_details.end_date)}
                                </p>
                              )}
                            </div>
                          )}
...
export default MentorActiveContractsPage;